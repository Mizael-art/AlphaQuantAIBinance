"""
Sobe um PostgreSQL real e efêmero (via pgserver, pip puro — sem apt/docker)
e define DATABASE_URL *antes* de qualquer módulo importar alphaquant_core,
evitando que o Base/metadata seja criado duas vezes com URLs diferentes.
"""
import atexit
import os
import tempfile
...
try:
    ...
    _datadir = tempfile.mkdtemp(prefix="aqx_test_pg_")
    ...
    # sem pgserver instalado: os testes que dependem de banco real são pulados
...
def _create_schema():
    ...
    from alphaquant_core.db import models  # noqa: F401 — registra as tabelas
    from alphaquant_core.db.session import Base, SessionLocal, engine

    Base.metadata.create_all(engine)

    # Espelha a migration 0002_seed_playbooks.py (não roda Alembic nos
    # testes — só recria o mesmo seed direto via ORM) para que os testes
    # de Quality Filter/Score exerçam o caminho real de leitura de
    # minimum_score/minimum_rr por playbook.
    db = SessionLocal()
    try:
