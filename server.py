...
def get_scan(
    symbols: str | None = Query(
        ...
        description=(
            ...
            f"ativos em config.DEFAULT_SCAN_SYMBOLS). Ignorado quando `universe=all_bybit`."
        ),
    ),
    universe: str = Query(
        default="watchlist",
        description=(
            "'watchlist' (padrão): usa `symbols` ou a watchlist fixa de "
            f"{len(DEFAULT_SCAN_SYMBOLS)} ativos. 'all_bybit': varre TODOS os perpétuos "
            "USDT negociáveis na Bybit em duas etapas -- filtro rápido de liquidez/atividade "
            "(sem candles) seguido de análise técnica completa só nos melhores candidatos "
            "(ver `top_n`). Use 'all_bybit' para não depender de uma lista fixa de ativos."
        ),
    ),
    top_n: int = Query(
        ...
        ge=1,
        le=150,
        description="Só usado com universe=all_bybit: quantos candidatos recebem análise técnica completa após o filtro rápido.",
    ),
    min_turnover_usdt: float = Query(
        default=SCAN_STAGE1_MIN_TURNOVER_USDT,
        ge=0,
        description="Só usado com universe=all_bybit: piso de liquidez (turnover 24h em USDT) para um ativo entrar no filtro rápido.",
    ),
